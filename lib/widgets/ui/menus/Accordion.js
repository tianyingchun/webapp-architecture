enyo.kind({
    ...
    rendered: enyo.inherit(function(sup) {
        return function() {
            ...
            this.highlightMenuItem(this.selectedKey);
        };
    }),
    ...
    //*@public check if current item need to collapsed or opened while first loading accordion menu.
    highlightMenuItem: function(selectedKey) {
        this.zLog("find matched selected menu item...");
        var $controls = this.getControls();
        for (var i = 0; i < $controls.length; i++) {
            var $item = $controls[i];
            //widgets.menus.accordion.Item->searchMenuItem
            var $matchedMenuItem = $item.searchMenuItem(selectedKey);
            ...
            for (var i = 0; i < childs.length; i++) {
                ...
                components.push({
                    ...
                    components: [{
                        tag: "a",
                        attributes: {
                            href: this.getLocationHash(child)
                        },
                        content: child[this.itemNameField]
                    }]
                });
            };
        }
    ...
    viewChanging: function(inSender, inEvent) {
        ...
        if (linkNode) {
            ...
            // make sure it always redirect to new hash location.
            window.location.href = linkNode.attributes.href;
        }
...
enyo.kind({
    ...
    //* @protected
    handlers: {
        //ontap: "tap"   // latest updated disable tap event each menu accordion change user hash changae event notify it toogle
    },
    create: enyo.inherit(function(sup) {
        return function() {
            sup.apply(this, arguments);
    ...
    tapHandler: function(inSender, inEvent, silent, linkNode) {
...
enyo.kind({
    ...
    toggleItem: function(silent, linkNode) {
        this.header.tapHandler(undefined, undefined, silent, linkNode);
    },
    //*@protected.
