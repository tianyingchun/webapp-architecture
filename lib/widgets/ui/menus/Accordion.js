enyo.kind({
	...
	highlightMenuItem: function () {
		...
		for (var i = 0; i < $controls.length; i++) {
			var $item = $controls[i];
			//widgets.menus.accordion.Item->highlightChildItem
			if($item.highlightChildItem(this.currentHash)){
				break;
			}
		};
	},
...
enyo.kind({
	...
	create: enyo.inherit(function (sup) {
		return function () {
			...
			var contentComponent = { name:"accordionItemContent",showing: !this.collapsed, kind: "widgets.menus.accordion.ItemContent", components: this.contentComponents};
	...
	//*@public
	highlightChildItem: function (hash) {
		var $childItems = this.$.accordionItemContent.getControls();
		var find = false;
		// first find accordion header 
		// second find accordion content.
		if(this.$.accordionItemHeader.getHref()==hash) {
