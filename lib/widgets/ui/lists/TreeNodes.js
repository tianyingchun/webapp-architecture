enyo.kind({
	...
	create: enyo.inherit(function (sup) {
		return function () {
			...
			this.createComponent([{ kind: "enyo.Node", onlyIconExpands:true, name:"treenode", content:this.rootNodeText,components: components }]);
			this.render();
		};	
	}),
		...
		if (enyo.isArray(source)) {
			for (var i = 0; i < source.length; i++) {
				...
				var convertItem = {
					...
					onlyIconExpands: true, // only allow icon folder expands.
				...
				if (this.selectedKey == convertItem.key) {
					...
					if (parent) {
						...
						parent.icon = "assets/images/arrow-open.png";
					}
				}
				if (level == 1) {
					delete convertItem.icon;
