enyo.kind({
	...
	createTabContent: function () {
		...
		for (var i = 0; i < this.contentComponents.length; i++) {
			var contentItem = {
				classes: this.selectedIndex==i? "active content-item" : "content-item",
				itemIndex: i,
				name : "tab-content-"+i,
				components: [this.contentComponents[i]]
			}; 
	...
	setActiveIndex: function (index) {
		...
		if ($content) {
			...
			this.zError("can't find content item index: `"+index+"` in SimpleTabs widget!");
		}
