enyo.kind({
	...
	itemSourceChanged: function (oldValue) {
		...
		// show tab content
		this.showTabContent(this.selectedItemKey);
	...
	showTabContent: function (itemKey) {
		...
		var langugeKey  = this.languageHljsMapping[itemKey];
		var hljsValue = "";
		if (!langugeKey) {
			this.zError("can't find matched language for hightlight js plugin!");
			hljsValue = selectedContent;
		} else {
			// If you know the language
			hljsValue = hljs.highlight(langugeKey, selectedContent).value;
		}
		this.$.content.destroyClientControls();
		this.$.content.createClientComponents([{ classes:"tab-content", allowHtml:true, content: hljsValue}]);
		this.$.content.render();
	},
	...
	show: enyo.inherit(function (sup) {
		return function () {
			sup.apply(this, arguments);
			// we need to reset control orignal status. if we hide tabControl before.
			this.initializeControlStatus();
			this.resetControlStatus();
		};
	})
