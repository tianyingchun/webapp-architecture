enyo.kind({
	...
	published: {
		...
		itemSource: [],
		// used to save current control appeared width
		offsetWidth : 0,
		//indicates current is slided shown. true is shown status.
		slideshown: false,
	},
	components: [
		{name:"header", id:"header-items", ontap: "toggleShowHidden"},	
	...
	selectedItemKey: "",
	// used to save current tab control last state's position.
	lastPosition:{
		left: 0,
		top: 0,
		marginLeft: 0
	...
	rendered: enyo.inherit(function (sup) {
		return function () {
			...
			// save control initialized position.
	...
	show: function () {
		if (this.itemSource.length) {
			this.fireAnimationStart(this.offsetWidth, this.getMaxWidth());
		} else {
	...
	getMaxWidth: function (){
		var tabControlWidth = this.getBounds().width || 0;
		// this.zLog("tabControlWidth: ", tabControlWidth)
	...
	stepTransition: function(fraction) {
		if (this.hasNode()) {
			var marginLeft = this.calcMarginLeft(fraction);
			this.applyStyle("margin-left",marginLeft +"px");
		}
	},
	//@procted, we can override this method to implemente different animation effections.
	calcMarginLeft: function (fraction) {
		...
		// this.zLog("last postion:",this.lastPosition,"fraction:", fraction, "now margin-left", marginLeft);
		return marginLeft;
	},
	finishTransition: function() {
		this.slideshown = !this.slideshown;
		// while animation finished, we need to save current control margin left . and position.
		this.lastPosition.marginLeft = parseInt(this.getComputedStyleValue("margin-left", 0));
		// save current control postion.
		this.lastPosition.left = this.getBounds().left;
		this.lastPosition.top = this.getBounds().top;
	}
});