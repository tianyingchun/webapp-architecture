...
enyo.kind({
	...
	published: {
		...
		displaySectionTitle: true,// default display section title header.
	...
	components: [
		{ name:"sectionHeader", classes:"section-header", components: [
			{name:"basicControls", classes:"basic-controls", components: [
				...
				{tag:"strong", content:" 显示标题:"},
				{name:"displaySectionTitle", scope:"ignore", style:"width: 30px;", kind:"onyx.Checkbox", checked: true},
				...
				{name:"removeBtn", ontap:"_removeSelfHandler", classes:"btn btn-danger", kind:"onyx.Button", content:"Remove"},
				{name:"copywidgetdata", ontap:"_copyWidgetDataHandler", classes:"btn btn-primary", kind:"onyx.Button", content:"复制"},
			]},
	...
	modelChanged: function () {
		if(this.model === "view") { 
			...
			// if don't show section title.
			this.$.sectionHeader.setShowing(this.displaySectionTitle);
	...
	preInit: function () {
		this.$.displaySectionTitle.setValue(this.displaySectionTitle);
	...
	getSectionJSONResult: function () {
		return {
			...
			displaySectionTitle: this.$.displaySectionTitle.getValue(),
	...
	// copy current widget data structure.
	_copyWidgetDataHandler: function (inSender, inEvent) {
		var result = this.getSectionJSONResult();
		window.pasteSupport.save("_widget_section_data_", result);
		return true;
	},
	//*@protected.
