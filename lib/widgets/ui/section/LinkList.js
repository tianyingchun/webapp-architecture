enyo.kind({
	...
	classes:"widget-section-linklist",
	...
	_drawLink: function (rowIndex, linkItem) {
		...
		if (this.model =="view") {
			...
			enyo.forEach(this.linkTarget, function (item) {
				var _new = {
					_id: item.id+rowIndex, // NOTE  we can't use `id` for each components, it will result component id repeated. use `_id` instead.
			...
			item ={ classes:"link-item", components: [
				...
				{rowIndex: rowIndex, dataKey:"target", key:"_id", kind:"widgets.forms.DropdownList", menuItemComponents:_target, defaultTitle:"--选择Target--"},
				{rowIndex: rowIndex, dataKey:"linkIcon", key:"_id", kind:"widgets.forms.DropdownList", menuItemComponents:_icons, defaultTitle:"--选择ICON--"},
