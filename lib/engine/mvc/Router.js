...
enyo.kind({
	...
	//@override 
	//Note: if we usually we need to encodeURIComponent for parameters,
	//Make sure that use decodeURIComponent to decode these parameters.
	addRoute: function (route) {
		var statics = this._staticRoutes;
		var dynamic = this._dynamicRoutes;
		var regex, token = /\:[a-zA-Z0-9]*/g;
		if (true === route['default']) {
			this.defaultRoute = route;
		}
		else if (token.test(route.path)) {
			regex = new RegExp(route.path.replace(token, "([a-zA-Z0-9-_%]*)"));
			route.regex = regex;
			dynamic.push(route);
		} else {
	...
	findHandler: function (route) {
		if (route && enyo.isObject(route)) {
			var actionVerb = enyo.uncap(route.action) || "index";
