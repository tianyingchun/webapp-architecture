...
enyo.kind({
	...
	receiveMessage: function (viewModel, viewAction) {
		this.zLog("received message(viewModel):", viewModel, viewAction);
	...
	/**
	 * Get all matched parameters for current routes.
	 * @param  {string} key parameter name. product/:page/:id" 
	 */
	getViewData: function (key) {
		if (this.viewData) {
			return this.viewData[key];
		} else {
			this.zError("the viewData of current view is null");
		}
	},
	/**
