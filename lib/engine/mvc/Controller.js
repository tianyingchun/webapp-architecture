...
enyo.kind({
	...
	//*@protected, should be invoke by each view before it's showing..
	_hasPassedSecurityValidation: function () {
		if (this.get("security")) {
			if(!Master.session.hasAuthorization()){
				//redirect to home page.
				this.zLog("controller security redirect! ");
				return false;
			}
		}
		return true;
	 ...
	 * Note: if in the same page, we have use the same View in multiplace in current page, e.g.  View1 used in leftDock, and used in MainContent,
	 * it will confilict, maybe need to improve _viewMapping implements
	 * TODO: improve compatibility for the same view used in mutiliple place of the same page.
	 */
	...
	notifyView: function (viewKindName, viewModel, viewData) { 
		// add controller security verification.
		if(!this._hasPassedSecurityValidation()){
			this.zLog("controller security validate failed");
