(function(enyo) {
	...
	enyo.setPath("Master.ModelSupport", {
		...
		fetch: enyo.inherit(function (sup) {
			return function (options) {
				...
				if (requestApi) {
					...
					this.zLog("do GET request...");
					// invoke enyo.Model. fetch Opts:{ "Method: GET" }
					sup.call(this, opts);
				} else {
		...
		destroy: enyo.inherit(function (sup) {
			return function (options) {
				// do some changes for option config.
				var opts = enyo.clone(this._defaultApiOpts);
				
				this.zLog("do DELETE request...");
				sup.call(this, opts);
			};
		}),
		/**
