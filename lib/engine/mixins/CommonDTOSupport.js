(function(enyo) {
    ...
    enyo.setPath("Master.CommonDTOSupport", {
        ...
        apiDefaultDTO: function(data) {
            ...
            // do some basic convert. if we don't provider data structure {retcode:xx,message:xx, info:xxx}
            // we must consider this scene, be cause in enyo.Model.js->contructor will invoke this.parse(data), if we 
            // return undefined it will ignore data {id:""}
        ...
        apiExceptionDTO: function(apiOtps, res) {
            ...
            if (errorBody) {
                info = enyo.json.parse(errorBody);
            }
            this.zError("apiOpts: ", apiOtps, "res:", res);
            // Uniform error data.
            var retCode = ErrorCode[info.RetCode] || ErrorCode[status] || status;
