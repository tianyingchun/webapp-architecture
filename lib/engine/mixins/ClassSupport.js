...
(function () {
	...
	enyo.setPath("Master.ClassSupport", {
		...
		/**
		 * Get platform types for current device.
		 * 
		 * @return {tablet, phone, browser} 
		 */
		...
		/**
		 * Check if there are connected network (online offline?)
		 * @public
		 * @return {Boolean}
		 */
		hasNetwork: function () {
		...
		zError: function () {
			...
			if (this.isNative()) {
				...
				var error = this.error || enyo.error;
