(function () {
	...
	/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
	var __securityKey = "PAF@123456789";
	enyo.mixin(utility, {
		//AES 
		AES: {
			encrypt: function (message) {
				if(enyo.isObject(message)) {
					message = enyo.json.stringify(message);
				}
				var encrypted = Aes.Ctr.encrypt(message,__securityKey, 128);
				return encrypted;
			},
			decrypt: function (encrypted) {
    			var decrypted = Aes.Ctr.decrypt(encrypted,__securityKey,128);
    			return decrypted;
			}
		}
	});
